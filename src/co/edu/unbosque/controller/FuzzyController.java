...
public class FuzzyController {
		...
		this.view.getCalcularButton().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					String resultado = model.calcularCalidadSueno(horasSueno, tiempoConciliar, sensacionDescanso,
							puntosPreguntas, edad);
	...
	public void escribir(String frase) {
		model.leerTxt();
		if (model.getTexto() != null) {
			model.actualizarTxt(model.getTexto() + "\n" + frase);
		} else {
			model.actualizarTxt(frase);
		}
	}
}